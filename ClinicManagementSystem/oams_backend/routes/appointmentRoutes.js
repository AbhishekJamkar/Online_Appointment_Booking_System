...
router.get("/checkAvailability", async (req, res) => {
  try {
    ...
    if (!service) {
      console.error("Doctor not found for serviceId:", serviceId);
    ...
    if (isNaN(serviceDuration) || serviceDuration <= 0) {
      console.error("Invalid Appointment duration:", serviceDuration);
...
const sendAppointmentRequestConfirmationEmail = async (
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: "techplementwebteam@gmail.com", 
        pass: "zopx wavl cptb ujnb", 
      },
    });

    const mailOptions = {
      from: "techplementwebteam@gmail.com", 
      to: currUserEmail,
...
const sendAppointmentConfirmationEmail = async (
  ...
  try {
    const service = await Service.findById(serviceId);
    if (!service) {
      console.error("Appointment not found for serviceId:", serviceId);
      return;
    }
    ...
    const mailOptions = {
      ...
      to: userEmail,
      subject: "Appointment Confirmation",
      text: `Your Appointment has been requested by ${currUserEmail}.\nDate: ${date}\nTime: ${time}`,
    };
